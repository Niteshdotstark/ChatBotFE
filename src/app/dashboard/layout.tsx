...
export default function DashboardLayout({
    ...
    return (
        <DashboardContext.Provider value={contextValue}>
            <div className="h-[calc(100vh-80px)] flex bg-gray-50 font-roboto" style={{ height: 'calc(100vh - 80px)' }}>
                <style jsx global>{customStyles}</style>

                {/* Sidebar Navigation */}
                <nav className="w-72 bg-gray-900 h-full flex flex-col dotstark-shadow-lg z-20 sticky top-0" style={{ height: '100%' }}>
                    <div className="p-6 flex-1 flex flex-col">
                        <div className="space-y-2 mt-4">
                            <Link href="/dashboard" className={`nav-link-style ${pathname === '/dashboard' ? 'active' : 'text-secondary hover:text-white'}`}>
                            ...
                            <Link href="/dashboard/chat" className={`nav-link-style ${pathname === '/dashboard/chat' ? 'active' : 'text-secondary hover:text-white'}`} style={planType === 'expired' ? { pointerEvents: 'none', opacity: 0.5 } : {}}>
                            ...
                            <Link href="/dashboard/organizations" className={`nav-link-style ${pathname === '/dashboard/organizations' ? 'active' : 'text-secondary hover:text-white'}`}>
                            ...
                            <Link href="/dashboard/knowledge" className={`nav-link-style ${pathname === '/dashboard/knowledge' ? 'active' : 'text-secondary hover:text-white'}`}>
                ...
                <div className="flex-1 flex flex-col">
                    <header className="bg-white dotstark-shadow border-b border-gray-200 px-5 py-4">
                        <div className="flex justify-between items-center">
                            <div>
                                <h2 className="text-2xl font-nunito font-bold text-secondary fw-900 uppercase tracking-wide">
                                    {currentPageTitle}
                                </h2>
                            ...
                            <div className="d-flex align-items-center gap-4">
                                <span className="bg-orange-100 text-orange-800 font-nunito font-bold px-4 py-2 rounded-full text-sm uppercase tracking-wider">
                                    ONLINE
                                </span>
                                <Link href="/dashboard/chat" className="dotstark-gradient text-white px-5 py-3 pill-button font-nunito font-bold text-sm shadow-md flex items-center" style={planType === 'expired' ? { pointerEvents: 'none', opacity: 0.5 } : {}}>
                                    <svg className="w-5 h-5 me-2" fill="currentColor" viewBox="0 0 20 20"><path d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z" /></svg>
                                    OPEN CHAT
                                </Link>
                            </div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {/* Main Error Banner with INCREASED PADDING (Request #3) */}
                        {error && (
                            <div className="bg-red-100 border border-red-400 text-red-700 px-5 py-5 rounded-xl mx-4 mt-4 dotstark-shadow" role="alert">
                ...
                {showTenantCreationForm && (
                    <div className="modal-overlay active d-flex align-items-center justify-content-center z-50 p-4">
                            ...
                            <div className="space-y-6">
                                {createStep === 1 && (
                                    <div>
                                        ...
                                        <input type="text" id="newTenantName" placeholder="e.g., My Company Inc." value={newTenantName} onChange={(e) => setNewTenantName(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} />
                                ...
                                {createStep === 2 && (
                                    <div className="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" id="newTenantFbUrl" placeholder="https://www.facebook.com/your-org" value={newTenantFbUrl} onChange={(e) => setNewTenantFbUrl(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg" disabled={isLoading} />
                                        </div>
                                        <div>
                                            ...
                                            <input type="text" id="newTenantInstaUrl" placeholder="https://www.instagram.com/your-org" value={newTenantInstaUrl} onChange={(e) => setNewTenantInstaUrl(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} />
                                        </div>
                                    </div>
                                )}
                                {createStep === 3 && (
                                    <div className="space-y-4">
                                        ...
                                        <div><label htmlFor="newTenantFbVerifyToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2  mt-2 uppercase tracking-wide">FB/Insta Verify Token</label><input type="text" id="newTenantFbVerifyToken" placeholder="Your webhook verification token" value={newTenantFbVerifyToken} onChange={(e) => setNewTenantFbVerifyToken(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg" disabled={isLoading} /></div>
                                        <div><label htmlFor="newTenantFbAccessToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 mt-2  uppercase tracking-wide">Facebook Page Access Token</label><input type="text" id="newTenantFbAccessToken" placeholder="EAAKgyth..." value={newTenantFbAccessToken} onChange={(e) => setNewTenantFbAccessToken(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} /></div>
                                        <div><label htmlFor="newTenantInstaAccessToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 mt-2 uppercase tracking-wide">Instagram Access Token</label><input type="text" id="newTenantInstaAccessToken" placeholder="IGAAbUo..." value={newTenantInstaAccessToken} onChange={(e) => setNewTenantInstaAccessToken(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} /></div>
                                    </div>
                                )}
                                {createStep === 4 && (
                                    <div className="space-y-4">
                                        <h3 className="text-lg font-nunito font-bold text-gray-700 border-b pb-2">Telegram Setup (Optional)</h3>
                                        <div><label htmlFor="newTenantTelegramBotToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 mt-2 uppercase tracking-wide">Telegram Bot Token</label><input type="text" id="newTenantTelegramBotToken" placeholder="8351802422:AAH74q..." value={newTenantTelegramBotToken} onChange={(e) => setNewTenantTelegramBotToken(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} /></div>
                                        <div><label htmlFor="newTenantTelegramChatId" className="block text-sm font-nunito font-bold text-gray-700 mb-2 mt-2uppercase tracking-wide">Telegram Chat ID (Internal Use)</label><input type="text" id="newTenantTelegramChatId" placeholder="Optional: 123456789" value={newTenantTelegramChatId} onChange={(e) => setNewTenantTelegramChatId(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} /></div>
                                    </div>
                                )}
                            </div>
                            <div className="mt-6 d-flex justify-content-between gap-3">
                ...
                {editingTenant && (
                    <div className="modal-overlay active d-flex align-items-center justify-content-center z-50 p-4">
                        <div className="modal-content bg-white rounded-3xl dotstark-shadow-lg p-4 w-full max-w-lg mx-auto overflow-y-auto max-h-[90vh] relative">
                            ...
                            <div className="space-y-6">
                                {editStep === 1 && (<div><h3 className="text-xl font-nunito font-bold text-gray-900 border-b pb-2">Basic Info</h3><label htmlFor="updatedName" className="block text-sm font-nunito font-bold text-gray-700 mb-2 mt-4 uppercase tracking-wide">Organization Name</label><input type="text" id="updatedName" value={updatedName} onChange={(e) => setUpdatedName(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} />{error && (<p className="text-red-500 text-sm mt-2">{error}</p>)}</div>)}
                                {editStep === 2 && (<div className="space-y-4"><h3 className="text-lg font-nunito font-bold text-gray-700 border-b pb-2">Social Links</h3><div><label htmlFor="updatedFbUrl" className="block text-sm font-nunito font-bold text-gray-700 mb-2  mt-2 uppercase tracking-wide">Facebook URL</label><input type="text" id="updatedFbUrl" placeholder="https://www.facebook.com/your-org" value={updatedFbUrl} onChange={(e) => setUpdatedFbUrl(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg" disabled={isLoading} /></div><div><label htmlFor="updatedInstaUrl" className="block text-sm font-nunito font-bold text-gray-700 mb-2 mt-2 uppercase tracking-wide">Instagram URL</label><input type="text" id="updatedInstaUrl" placeholder="https://www.instagram.com/your-org" value={updatedInstaUrl} onChange={(e) => setUpdatedInstaUrl(e.target.value)} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " disabled={isLoading} /></div></div>)}
                                {editStep === 3 && (<div className="space-y-4"><h3 className="text-lg font-nunito font-bold text-gray-700 border-b pb-2">Webhooks & Links</h3><div><label htmlFor="updatedFbVerifyToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 uppercase tracking-wide">FB/Insta Verify Token</label><input type="text" id="updatedFbVerifyToken" placeholder="Verification token" value={updatedFbVerifyToken} onChange={(e) => setUpdatedFbVerifyToken(e.target.value)} disabled={isLoading} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg" /></div><div><label htmlFor="updatedFbAccessToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 uppercase tracking-wide">Facebook Page Access Token</label><input type="text" id="updatedFbAccessToken" placeholder="EAAKgyth..." value={updatedFbAccessToken} onChange={(e) => setUpdatedFbAccessToken(e.target.value)} disabled={isLoading} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " /></div><div><label htmlFor="updatedInstaAccessToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 uppercase tracking-wide">Instagram Access Token</label><input type="text" id="updatedInstaAccessToken" placeholder="IGAAbUo..." value={updatedInstaAccessToken} onChange={(e) => setUpdatedInstaAccessToken(e.target.value)} disabled={isLoading} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " /></div></div>)}
                                {editStep === 4 && (<div className="space-y-4"><h3 className="text-lg font-nunito font-bold text-gray-700 border-b pb-2">Telegram Setup</h3><div><label htmlFor="updatedTelegramBotToken" className="block text-sm font-nunito font-bold text-gray-700 mb-2 uppercase tracking-wide">Telegram Bot Token</label><input type="text" id="updatedTelegramBotToken" placeholder="8351802422:AAH74q..." value={updatedTelegramBotToken} onChange={(e) => setUpdatedTelegramBotToken(e.target.value)} disabled={isLoading} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg" /></div><div><label htmlFor="updatedTelegramChatId" className="block text-sm font-nunito font-bold text-gray-700 mb-2 uppercase tracking-wide">Telegram Chat ID</label><input type="text" id="updatedTelegramChatId" placeholder="123456789" value={updatedTelegramChatId} onChange={(e) => setUpdatedTelegramChatId(e.target.value)} disabled={isLoading} className="w-full bg-gray w-full px-4 py-3 border-2 border-light rounded-5 transition-colors text-lg-3 py-3 border-2 border-gray-200 rounded-xl transition-colors text-lg " /></div></div>)}
                            </div>
                            <div className="mt-6 d-flex justify-content-between gap-3">
